# SMTP Relay Architecture & Operations – US\-West\-2
...
| `inf-ngxrelay101` | Datacenter NGINX TCP relay |
| `inf-relay101` | Postfix relay host |
...
**Purpose:**  
Performs TCP 25 / 587 forwarding to inf-relay101.cloud.lcl

```
Hostname:      inf-ngxrelay101.cloud.lcl
Private IP:    172.29.20.68
...
```
Hostname:      inf-relay101.cloud.lcl
Instance ID:   i-0be7224e91e5b785a
Private IP:    172.29.249.50
...
| --- | --- | --- |
| [email-smtp.us-west-2.amazonaws.com](http://email-smtp.us-west-2.amazonaws.com/) | 23.249.223.50 | [relay101\_pools](https://361362055558-ss6yhern.us-west-2.console.aws.amazon.com/ses/home#/dedicated-ips/relay101_pools) |
...
| Host | Access |
| --- | --- |
| NGINX (`inf-ngxrelay101`) | **SSH/Putty **via localadmin account |
| Relay (`inf-relay101`) | **RDP **via svc-wsl-relay account |
...
| Item | Value |
| --- | --- |
| IAM User | `ses-relay-service-usw2` |
...
Regardless of which region is chosen, the skill adds the domain to `relay_domains` on **both** Postfix relays — `inf-relay101` (us-west-2, this document) and `inf-relay001` (us-east-1, see the [US-East-1 doc](SMTP%20Relay%20Architecture%20%26%20Operations%20%E2%80%93%20US-East-1.md)) — for failover. It creates the SES verified identity (Easy DKIM), saves the DKIM CNAME records to a CSV in Downloads to send to the client, and inserts the domain alphabetically into the Government section of `relay_domains` on each relay, then reloads Postfix and validates the entry.
...
1. RDP to inf-relay101.cloud.lcl
...
```
SMTP Server: relay101.aspgov.com
SMTP Port:   25
Authentication: None
```
...
```
SMTP Server: inf-relay101.cloud.lcl
SMTP Port:   25
Authentication: None
```
